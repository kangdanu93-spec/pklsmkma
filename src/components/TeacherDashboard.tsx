...
export default function TeacherDashboard({ teacher, instansiList, refreshCounter }: TeacherDashboardProps) {
  ...
  const [isSubmittingMon, setIsSubmittingMon] = useState(false);
  ...
  const handleSaveMonitoring = async (e: React.FormEvent) => {
    ...
    // Prevent duplicate monitoring submission for same instansi, date, and type
    const isDuplicate = monitorings.some(m => {
      const matchInstansi = m.id_siswa === monInstansiId || 
                            (m.nama_instansi && targetInstansi && m.nama_instansi.toLowerCase().trim() === targetInstansi.nama_instansi.toLowerCase().trim());
      const matchTipe = m.tipe_monitoring === monType || (m as any).tipe === monType;
      const matchTanggal = m.tanggal === monDate;
      return matchInstansi && matchTipe && matchTanggal;
    });

    if (isDuplicate) {
      alert(`Laporan ${monType} untuk instansi "${instansiName || 'ini'}" pada tanggal ${monDate} sudah pernah disubmit sebelumnya! Silakan periksa riwayat kunjungan.`);
      return;
    }
    
    try {
      setIsSubmittingMon(true);
      ...
      const newMon: TeacherMonitoring = {
        id: generateUUID(),
        id_guru: teacher.id || teacher.email || teacher.nomor_induk || '',
        nama_guru: teacher.nama || 'Guru Pembimbing',
        tanggal: monDate,
        jam_monitoring: monTime || new Date().toTimeString().split(' ')[0],
      ...
      if (res.success) {
        ...
        setMonitorings(resMon.data.filter(m => 
          m.id_guru === teacher.id || 
          m.nama_guru === teacher.nama || 
          m.id_guru === teacher.email || 
          m.id_guru === teacher.nomor_induk ||
        ...
        alert(res.error || 'Gagal menyimpan laporan monitoring. Mohon periksa kembali data Anda.');
      }
    } catch (err: any) {
      console.error('Error saving teacher monitoring:', err);
      alert('Terjadi kesalahan saat menyimpan laporan monitoring: ' + (err?.message || 'Error'));
    } finally {
      setIsSubmittingMon(false);
    }
  };
          ...
          <div className="lg:col-span-5 space-y-6">
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 space-y-5">
              ...
              <form onSubmit={handleSaveMonitoring} className="space-y-4">
                ...
                <button
                  type="submit"
                  disabled={isSubmittingMon}
                  className="w-full py-3 bg-indigo-600 hover:bg-indigo-700 disabled:bg-slate-400 disabled:cursor-not-allowed text-white text-xs font-bold rounded-xl transition-all shadow-md shadow-indigo-600/10 flex items-center justify-center gap-1.5"
                >
                  {isSubmittingMon ? (
                    <>
